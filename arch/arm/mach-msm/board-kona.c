...
#include <mach/msm8960-gpio.h>
	...
	defined(CONFIG_TOUCHSCREEN_SYNAPTICS_KONA)
...
#endif
#ifdef CONFIG_SENSORS_AK8963
static void akm_power_on(bool onoff);
#endif
...
void __init input_tsp_init(void)
{
	pr_info("[TSP] tsp init........\n");
...
static void irda_wake_en(bool onoff)
{
	...
	gpio_direction_output(GPIO_IRDA_EN, onoff);
	printk(KERN_ERR "%s: irda_en : %d\n", __func__, onoff);
}
...
static void irda_device_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_IRDA_WAKE, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
...
static struct akm8963_platform_data akm8963_pdata = {
	...
	.layout = 6,
	.outbit = 1,
	.gpio_DRDY	= GPIO_MAG_RDY,
	.gpio_RST	= GPIO_MAG_RST,
};
#endif
#if defined (CONFIG_YAS_MAG_DRIVER_YAS532)
	static struct yas_platform_data yas532_pdata = {
	.mag_orientation = 3,
};
#endif
...
static int __init sensor_device_init(void)
{
	...
	if(system_rev  < BOARD_REV03)
		yas532_pdata.mag_orientation = 6;
...
static void akm_power_on(bool onoff)
{
	if (onoff)
		...
		sensor_power_on_vdd(SNS_PWR_OFF, SNS_PWR_OFF);
}
#endif
...
int64_t acc_get_adc_value(void)
{
	int64_t value = 0;
	/*
		ADC_DATA_CH0 : X+/GPIO-4
		ADC_DATA_CH1 : X-/GPIO-6
		ADC_DATA_CH2 : Y+/GPIO-5
		ADC_DATA_CH3 : Y-/GPIO-7
		ADC_DATA_CH4 : IN0/GPIO-0
		ADC_DATA_CH5 : IN1/GPIO-1
		ADC_DATA_CH6 : IN2/GPIO-2
		ADC_DATA_CH7 : IN3/GPIO-3
	 */
	value = (int64_t)stmpe811_adc_get_value(0);
	return value;
}
...
static struct platform_device *kona_att_devices[] __initdata = {
	&msm_8960_q6_lpass,
	&msm_8960_q6_mss_fw,
	&msm_8960_q6_mss_sw,
	&msm_8960_riva,
...
static struct i2c_board_info bl_i2c_borad_info[] = {
	{
		I2C_BOARD_INFO("BL", 0x2C),
	},
};
#endif
...
static void __init samsung_kona_att_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#endif
	usb_switch_init();
	platform_add_devices(kona_att_devices,
			ARRAY_SIZE(kona_att_devices));
...
MACHINE_START(KONA, "SAMSUNG KONA")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = samsung_kona_att_init,
