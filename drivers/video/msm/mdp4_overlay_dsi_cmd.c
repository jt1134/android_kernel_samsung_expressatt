		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!wait_for_completion_timeout(&vctrl->vsync_comp,
		...
		/* Does not receive interrupt from MDP,
		Something wrong */
		panic("vsync_show_event interrupt missing");
	}
...
void mdp4_dsi_cmd_overlay(struct msm_fb_data_type *mfd)
{
	...
	if (atomic_read(&vctrl->suspend)) {
		...
		/*
		*	MIPI_RESUME_STATE = 0
		*	MIPI_SUSPEND_STATE = 1
		*/
		if ((mfd->display_on_status == 1) || poweroff_charging ) {
			mipi_dsi_clk_cfg(1);
			mdp_clk_ctrl(1);
			powerdown_req =1;
...
#if defined(CONFIG_FB_MSM_MIPI_CMD_PANEL_VSYNC)
	/*
	 * It was found during initial power testing that MDP clocks are
	 * still ON in sleep mode and increasing power consumption. The reason for
	 * it is the un-balanced enable/disable calls during initial power on.
	 *
	 * So disabling the MDP clocks to balance out this problem.
	 */
	if (powerdown_req) {
			/* This delay required to prevent garbage screen*/
			msleep(20);
			mdp_clk_ctrl(0);
			powerdown_req =0;
	}
